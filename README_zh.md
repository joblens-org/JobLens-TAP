...
# 运行测试（所有测试，包括 tests/ 和 internal/ 下的白盒测试）
make test
...
> 如果仅需编译二进制（不依赖 Docker），执行 `make build`，产物为 `bin/server`。
...
> 完整 API 文档参见 [doc/API_zh.md](./doc/API_zh.md)，管理 API 响应规范参见 [doc/management-api_zh.md](./doc/management-api_zh.md)
...
├── doc/                     # 文档
│   ├── API.md               # API 接口文档（英文）
│   ├── API_zh.md            # API 接口文档（中文）
│   ├── management-api.md    # 管理 API 响应规范（英文）
│   └── management-api_zh.md # 管理 API 响应规范（中文）
├── design.md                # 设计文档（仅供参考）
└── README.md
```
...
make test          # 运行全部测试（tests/ + internal/）
...
- **两类测试**:
  - `internal/**/*_test.go`：白盒单元测试，不依赖 ES
  - `tests/`：黑盒集成测试（`package tests`），依赖真实 ES
